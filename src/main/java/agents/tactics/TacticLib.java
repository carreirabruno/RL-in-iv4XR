...
//                     System.out.println("### setting a new exploration target: " + g) ;
	...
	public static Tactic randomWalk(String agentID) {

		Tactic rand_walky = action("Just walking around.")
				.do1((BeliefState belief) -> {
					belief.env().randomWalk(agentID);
					return belief ;
				})
				.lift() ;
		return rand_walky ;
	...
	public static Tactic dynamicNavigateTo_Bruno(String tacticname, Vec3 position) {
		return FIRSTof(
				forceReplanPath(),
				tryToUnstuck(),
				rawNavigateTo_(tacticname,position).lift(),
